...
def create_log_file(filename="analysis_results.txt"):
    """Create a log file to save analysis results."""
    with open(filename, 'w') as f:
        f.write("Cancer Mutation Analysis Results\n")
        f.write("===============================\n\n")
    return filename
...
def build_logistic_regression_model(X_train, X_test, y_train, y_test, feature_names, log_filename, C_values=None):
    ...
    write_to_log(log_filename, f"Best hyperparameter C: {grid_search.best_params_['C']}")
    ...
    # Convert feature_importance to string and write to log
...
def plot_roc_curve(y_test, y_pred_proba, log_filename):
    ...
    write_to_log(log_filename, "ROC curve has been saved to 'roc_curve.png'")
...
def plot_feature_importance(model, feature_names, log_filename):
    ...
    write_to_log(log_filename, "Feature importance plot has been saved to 'feature_importance.png'")
...
def plot_confusion_matrix(y_test, y_pred, log_filename):
    ...
    write_to_log(log_filename, "Confusion matrix has been saved to 'confusion_matrix.png'")
...
def analyze_mutational_signatures_and_immune_infiltration():
    ...
    model, y_pred, y_pred_proba, scaler = build_logistic_regression_model(X_train_selected, X_test_selected, y_train, y_test, selected_features, log_filename)
    
    # Visualization
    plot_roc_curve(y_test, y_pred_proba, log_filename)
...
if __name__ == "__main__":
    log_filename = create_log_file()
    write_to_log(log_filename, "Starting analysis of how mutational signatures shape immune cell infiltration...")
